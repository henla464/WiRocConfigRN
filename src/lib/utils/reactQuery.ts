...
export const setupReactQuerySubscriptionToDevice = (
  ...
  if (devices.has(deviceId)) {
    log.warn(`Already listening for device ${deviceId}`);
    return;
  }
  ...
  apiBackend.onPropertiesChanges(newData => {
    Object.entries(newData).forEach(([key, value]) => {
      ...
      log.info('Update value for', deviceId, key, parsedValue);
  ...
  apiBackend.onPunchesRecieved(newPunches => {
    queryClient.setQueryData<Punch[]>(
      ...
      (previousPunches = []) => {
        log.info(
          `Adding ${newPunches.length} new punch${
  ...
  apiBackend.onTestPunchesSent(sentPunches => {
    queryClient.setQueryData<TestPunch[]>(
      ...
      (previousPunches = [] as TestPunch[]) => {
        ...
        log.info(
          `Adding ${newPunches.length} new test punch${
...
export const updateQueryDataForDevice = (
  ...
  queryClient.setQueryData(getKey(deviceId, propertyName), current => {
    log.debug(`Setting ${propertyName}: ${type} = ${JSON.stringify(value)}`);
    if (propertyName === 'settings') {
      log.debug('Merging settings');
      log.debug('Current:', type, current, current?.settings);
      const updatedValue = {
        settings: [...(current?.settings ?? []), value],
      };
      log.debug('Updated:', type, updatedValue);
      return updatedValue;
    }
    return value;
  });
